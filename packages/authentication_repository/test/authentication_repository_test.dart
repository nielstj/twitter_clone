...
void main() {
  ...
  group('AuthenticationRepository', () {
    ...
    test('creates FirebaseAuth instance internally when not injected', () {
      expect(() => FirebaseAuthenticationRepository(), isNot(throwsException));
    });
    ...
    group('user', () {
      ...
      test('emits User when firebase user is not null', () async {
        ...
        verify(
          () => cache.write(
            key: FirebaseAuthenticationRepository.userCacheKey,
            value: user,
          ),
        ).called(1);
      });
    ...
    group('currentUser', () {
      test('returns User.empty when cached user is null', () {
        when(
          () => cache.read(key: FirebaseAuthenticationRepository.userCacheKey),
        ).thenReturn(null);
        expect(
